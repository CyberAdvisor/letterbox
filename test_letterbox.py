...
# ===========================================================================
# SECTION 23 — Physical Vault Transfer (File Path)
# ===========================================================================

section("23 · Physical Vault Transfer — File Path  [SECURITY]")

# Tests for _save_transfer_vault_to_file and _load_transfer_vault_from_file
# logic. These functions are tested at the vault layer (save/load) rather
# than via the UI functions directly, since the UI functions involve
# interactive input prompts.
#
# What is tested:
#   23.1  Transfer vault saved to file is loadable with the transfer phrase
#   23.2  Transfer vault file is encrypted (not plaintext)
#   23.3  Transfer vault from file survives reencrypt_vault (Bob's import)
#   23.4  Bob's vault from file import has complementary send_pads to Alice
#   23.5  Transfer vault file encrypted differently to personal vault
#         (different magic, different key derivation)
#   23.6  Wrong transfer phrase on file vault raises WrongPassphraseError
#   23.7  Truncated transfer vault file raises an error on load

d = tmpdir()
try:
    # Simulate Alice: generate vault, save as transfer vault to a file
    alice_salt   = generate_salt()
    alice_bundle = generate_bundle_id()
    alice_vault  = generate_vault(alice_bundle, ephemeral=False)
    transfer_phrase = generate_transfer_passphrase()

    # Write transfer vault to file (mirrors _save_transfer_vault_to_file)
    transfer_path = d / "letterbox_transfer.vault"
    alice_vault.is_transfer = True
    save_vault(alice_vault, transfer_path, transfer_phrase, alice_salt)
    alice_vault.is_transfer = False

    # 23.1: File is loadable with correct transfer phrase
    loaded_transfer = load_vault(transfer_path, transfer_phrase, is_transfer=True)
    check("23.1 Transfer vault saved to file loads successfully with transfer phrase",
          loaded_transfer.bundle_id == alice_bundle)

    # 23.2: File is encrypted (plaintext magic bytes absent from raw file)
    raw_bytes = transfer_path.read_bytes()
    check("23.2 Transfer vault file does not contain plaintext magic bytes",
          b"LBVAULT2" not in raw_bytes and b"LBTRANS2" not in raw_bytes,
          "magic bytes found in raw file -- file is not properly encrypted")

    # 23.3: Bob imports from file and reencrypts with his own passphrase
    bob_salt = generate_salt()
    bob_pass = "bob-file-import-passphrase"
    bob_vault_path = d / "bob_vault.dat"
    reencrypt_vault(loaded_transfer, bob_vault_path, bob_pass, bob_salt)
    bob_vault = load_vault(bob_vault_path, bob_pass)
    check("23.3 Bob's vault from file import loads correctly after reencrypt",
          bob_vault.bundle_id == alice_bundle)

    # 23.4: Alice and Bob have complementary send_pads (no overlap, full coverage)
    check("23.4 File transfer: Alice and Bob send_pads are complementary",
          len(alice_vault.send_pads & bob_vault.send_pads) == 0 and
          len(alice_vault.send_pads | bob_vault.send_pads) == PAD_COUNT,
          f"overlap={len(alice_vault.send_pads & bob_vault.send_pads)}, "
          f"union={len(alice_vault.send_pads | bob_vault.send_pads)}")

    # 23.5: Transfer vault file cannot be loaded as a personal vault
    # (different magic bytes and key derivation)
    check_raises(
        "23.5 Transfer vault file rejected when loaded as personal vault",
        WrongPassphraseError,
        load_vault, transfer_path, transfer_phrase, False  # is_transfer=False
    )

    # 23.6: Wrong transfer phrase raises WrongPassphraseError
    check_raises(
        "23.6 Wrong transfer phrase raises WrongPassphraseError",
        WrongPassphraseError,
        load_vault, transfer_path, "wrong phrase here", True
    )

    # 23.7: Truncated transfer vault file raises an error
    full = transfer_path.read_bytes()
    (d / "truncated.vault").write_bytes(full[:500])
    check_raises(
        "23.7 Truncated transfer vault file raises an error on load",
        Exception,
        load_vault, d / "truncated.vault", transfer_phrase, True
    )
