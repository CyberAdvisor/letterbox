...
# 2026-05-09  M.Lines   v3.1.0: make_subject_token, recover_pad_id, lookup_receive_pad_v3 added for v3
# ---------------------------------------------------------------------------

import hashlib
import hmac
import struct
from core.constants import PAD_COUNT, PAD_SIZE, SUBJECT_KEY_SIZE
...
# ---------------------------------------------------------------------------
# Subject token obfuscation
# ---------------------------------------------------------------------------

def make_subject_token(subject_key: bytes, pad_id: int) -> str:
    """
    Generate an obfuscated subject token for a pad ID.

    Uses HMAC-SHA256(subject_key, pad_id_bytes) truncated to
    SUBJECT_KEY_SIZE // 4 bytes, encoded as hex. Deterministic
    for a given subject_key and pad_id but reveals nothing about
    the sequential pad_id to an observer without the subject_key.

    Args:
        subject_key: shared 32-byte key from the vault
        pad_id:      pad index

    Returns:
        hex string of length 16 (8 bytes as hex)
    """
    pad_id_bytes = struct.pack('>I', pad_id)
    tag = hmac.new(subject_key, pad_id_bytes, hashlib.sha256).digest()
    return tag[:8].hex()
...
def recover_pad_id(
    ...
    """
    Recover the pad_id corresponding to a subject token.

    Searches sequentially from search_from for up to search_count
    candidates. Handles small sequence gaps gracefully.

    Args:
        subject_key:  shared 32-byte key from the vault
        token:        hex subject token from the email subject
        search_from:  starting pad_id to try
        search_count: how many candidates to try

    Returns:
        pad_id if found, None if not found in search range
    """
    for pad_id in range(search_from, search_from + search_count):
        if pad_id >= PAD_COUNT:
            break
        if make_subject_token(subject_key, pad_id) == token:
            return pad_id
    return None
...
def lookup_receive_pad_v3(
    pad_id: int,
    vault,
) -> bytes:
    """
    Look up pad bytes for a received message (v3, no history).

    Simplified version of lookup_receive_pad that does not require
    message history. In v3 (ephemeral only) there is no history
    database. Replay detection relies solely on the pad bitmap.

    Args:
        pad_id: pad ID recovered from subject token
        vault:  the VaultData for this contact
...
PAD_WARNING_LEVELS = [
    ( 10, "CRITICAL: Under 10 pads remaining."),
    ( 50, "Under 50 pads remaining. Send a message to arrange exchange."),
    (100, "Under 100 pads remaining. Exchange a new vault urgently."),
    (200, "Under 200 pads remaining. A new vault exchange is needed."),
    (500, "Under 500 pads remaining. Plan a new vault exchange soon."),
]
