...
# 2026-05-09  M.Lines   v3.1.0: recv_sequence field added for v3
# ---------------------------------------------------------------------------
...
_OFFSET_RECV_SEQ       = 61   # 8 bytes

CONFIG_FILE_SIZE       = 69   # total bytes
...
def read_config(path: Path) -> ConfigData:
    ...
    recv_sequence      = struct.unpack_from('>Q', raw, _OFFSET_RECV_SEQ)[0] if len(raw) >= 69 else 0
...
def write_config(path: Path, config: ConfigData) -> None:
    ...
    struct.pack_into('>Q', raw, _OFFSET_RECV_SEQ,     config.recv_sequence)
...
def get_recv_sequence(path: Path) -> int:
    """Return the last known receive sequence counter. Returns 0 if never set."""
    try:
        config = read_config(path)
        return config.recv_sequence
    except Exception:
        return 0


def update_recv_sequence(path: Path, sequence: int) -> None:
    """Update the receive sequence counter."""
    config = read_config(path)
    if sequence > config.recv_sequence:
        config.recv_sequence = sequence
        write_config(path, config)
