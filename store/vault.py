...
# 2026-05-08  M.Lines   v3.0:   VaultData.subject_key added; vault format version 3
# 2026-05-09  M.Lines   v3.1.0: subject_key, token_table added; vault format version 3
# ---------------------------------------------------------------------------
...
from util.random import random_bytes, generate_pad_data, generate_subject_key
...
class VaultData:
    ...
    def __init__(
        ...
        self.subject_key  = subject_key if subject_key is not None else random_bytes(SUBJECT_KEY_SIZE)
        # Token lookup table: {token_hex: pad_id} built at generation/load time
        self.token_table  = {}
...
def _build_token_table(vault: VaultData) -> bytes:
    """
    Serialise the token lookup table.

    For every pad in the vault, compute HMAC(subject_key, pad_id)
    and store the first SUBJECT_TOKEN_BYTES bytes at position
    pad_id * SUBJECT_TOKEN_BYTES. Result is a flat byte array of
    PAD_COUNT * SUBJECT_TOKEN_BYTES bytes.

    Receiving side deserialises this into a dict for O(1) lookup.
    """
    import hashlib
    import hmac as _hmac
    import struct

    table = bytearray(PAD_COUNT * SUBJECT_TOKEN_BYTES)
    for pad_id in range(PAD_COUNT):
        pad_id_bytes = struct.pack('>I', pad_id)
        tag = _hmac.new(vault.subject_key, pad_id_bytes, hashlib.sha256).digest()
        offset = pad_id * SUBJECT_TOKEN_BYTES
        table[offset:offset + SUBJECT_TOKEN_BYTES] = tag[:SUBJECT_TOKEN_BYTES]
    return bytes(table)


def _load_token_table(table_bytes: bytes) -> dict:
    """
    Deserialise token table bytes into {token_hex: pad_id} dict.
    O(1) lookup on receive.
    """
    result = {}
    for pad_id in range(PAD_COUNT):
        offset = pad_id * SUBJECT_TOKEN_BYTES
        token_bytes = table_bytes[offset:offset + SUBJECT_TOKEN_BYTES]
        result[token_bytes.hex()] = pad_id
    return result
...
def _build_plaintext(
    ...
    subject_key_bytes = vault.subject_key
    token_table_bytes = _build_token_table(vault)

    header = header + subject_key_bytes + token_table_bytes
...
def load_vault(
    ...
    # Parse subject_key (v3+)
    subject_key = None
    token_table_bytes = None
    if version >= 3:
        if offset + SUBJECT_KEY_SIZE <= len(plaintext) - VAULT_PAD_DATA_SIZE:
            subject_key = bytes(plaintext[offset:offset + SUBJECT_KEY_SIZE])
            offset += SUBJECT_KEY_SIZE
        if offset + VAULT_TOKEN_TABLE_SIZE <= len(plaintext) - VAULT_PAD_DATA_SIZE:
            token_table_bytes = bytes(plaintext[offset:offset + VAULT_TOKEN_TABLE_SIZE])
            offset += VAULT_TOKEN_TABLE_SIZE
    ...
    if token_table_bytes is not None:
        vault.token_table = _load_token_table(token_table_bytes)
    else:
        vault.token_table = _load_token_table(_build_token_table(vault))
    return vault
...
def generate_vault(bundle_id: int, ephemeral: bool = False) -> VaultData:
    ...
    It produces ~5MB of random bytes and may take 20-30 seconds
