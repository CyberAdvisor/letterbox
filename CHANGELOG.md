...
## 2.0 — Physical Vault Transfer and Setup UI Overhaul — 3 May 2026

FEATURE — major release.

**Physical vault transfer via file (AirDrop / Files app)**

The vault exchange no longer requires a Posteo upload. During setup,
both parties choose their transfer method immediately after choosing
their role (Generate / Import):

  1  Posteo  -- upload and download over IMAP (existing behaviour)
  2  File    -- save to file, share via AirDrop or Files app

**File transfer path (Alice):**
After vault generation and passphrase, Letterbox saves the encrypted
transfer vault to `letterbox_transfer.vault` in the data directory
and displays the transfer phrase. Alice shares the file manually via
AirDrop or the Files app and provides the transfer phrase to Bob.
Posteo credentials are collected separately afterwards for ongoing
message exchange.

**File transfer path (Bob):**
Bob receives the vault file from Alice, saves it to the letterbox
folder, runs Letterbox, chooses File transfer, enters the file path
(default is the data directory) and transfer phrase, decrypts the
vault, sets his passphrase, then enters Posteo credentials for
ongoing message exchange. The transfer vault file is deleted after
successful import.

**Security:** The transfer vault is always encrypted with 2,000,000
KDF iterations and the six-word transfer phrase regardless of transfer
method. File transfer eliminates the Posteo server exposure window
entirely -- the vault never touches any server. This is the preferred
path for users with serious threat models.

**Setup UI overhaul**

The setup flow has been redesigned. Key changes:

- Role choice reworded: "Generate vault -- you are setting up first"
  and "Import vault -- your contact set up first"
- Transport choice added as a second screen immediately after role
  choice, before any other questions
- Step-preview list removed from Alice's flow -- screens follow
  directly without announcing what is coming
- Encryption mode now uses 1/2 numeric choice instead of y/n
- "Tell your contact" screen simplified: emphasis on transfer phrase,
  Posteo details listed but de-emphasised in the file path
- Confirmation loop ("Have you copied these?") replaced with a
  single Press Enter prompt
- Bob's "four words" bug fixed -- was incorrectly described as four
  words; the transfer phrase is six words
- Posteo credential entry decoupled from vault transfer in the file
  path via new _setup_message_transport() helper
- "Verbally -- in person or video call only" replaced with "provide"
  throughout -- does not prescribe the channel

**New functions in main.py:**
- _save_transfer_vault_to_file(): encrypts and writes transfer vault
- _load_transfer_vault_from_file(): prompts for path, validates, reads
- _setup_message_transport(): collects and saves Posteo credentials;
  used by both parties on the file transfer path
