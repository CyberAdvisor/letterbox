...
def make_subject(bundle_id: int, pad_id: int) -> str:
    """
    Build the email subject line from bundle_id and pad_id.
    Used for searching and identification.
    """
    return f'msg-{bundle_id:08x}-{pad_id:08x}'


def parse_subject(subject: str) -> tuple:
    """
    Extract bundle_id and pad_id from an email subject line.

    Returns (bundle_id, pad_id) as integers, or (None, None)
    if the subject does not match the expected format.
    """
    try:
...
def post_message(
    ...
    subject = make_subject(bundle_id, pad_id)
...
def collect_messages(
    bundle_id:      int,
    credentials:    CredentialsData,
    skip_pad_set:   set   = None,
) -> list:
    """
    Collect all pending transmissions from the shared folder.

    Searches for messages with subjects matching the bundle ID,
    downloads each, decodes the transmission, and deletes the
    message from the server after successful download.

    Messages whose pad ID falls within skip_pad_range are our
    own sent messages -- they are deleted from the server but
    not returned to the caller.

    Args:
        bundle_id:      vault bundle ID to search for
        credentials:    Posteo account credentials
        skip_pad_set:   set of pad IDs we sent -- skip these

    Returns:
        list of raw transmission bytes (each TRANSMISSION_SIZE long)
        May be empty if no messages are waiting.
    ...
    try:
        ...
        try:
            ...
            for msg_id in ids:
                try:
                    # Check subject before downloading
                    # If pad ID is in our send range, this is our
                    # own message -- delete it and skip
                    if skip_pad_set is not None:
                        ...
                        if status2 == 'OK' and hdr and hdr[0]:
                            ...
                            for line in hdr_text.splitlines():
                                if line.lower().startswith('subject:'):
                                    ...
                                    break
                            _, pid_from_subject = parse_subject(subject)
                            if pid_from_subject is not None:
                                if pid_from_subject in skip_pad_set:
                                    # Our own message -- delete and skip
...
def download_vault(credentials: CredentialsData) -> bytes:
    ...
    except (VaultDownloadError, ConnectionError):
        raise
    except Exception as e:
        raise VaultDownloadError(
